...
def main():
	...
	parser.add_argument("--path", help="Full path to the CSV file")
	args = parser.parse_args()

	if len(sys.argv) <= 1:
		print('pcap2Ts.py --path full path to the csv')
		exit(1)

	outfile = "tsreadypcap" + datetime.datetime.now().strftime('%Y%m%d%H%M%S') 
